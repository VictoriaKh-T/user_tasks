...
public abstract class AbstractDao<T, I extends Serializable> {
    ...
    public T save(T entity) {
        ...
        try {
            ...
            throw new DataException("Can't create entity: " + entity, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public Optional<T> findById(I id) {
        try (Session session = sessionFactory.openSession()) {
            ...
            throw new DataException("Can't get entity: " + clazz.getSimpleName()
                    + " by id " + id, e);
        }
    }
    ...
    public void delete(I id) {
        ...
        try {
            session = sessionFactory.openSession();
            transaction = session.beginTransaction();
            session.remove(session.load(clazz, id));
            transaction.commit();
        } catch (Exception e) {
            if (transaction != null) {
                transaction.rollback();
            }
            throw new DataException("Can't delete entity by id: " + id, e);
        } finally {
            if (session != null) {
                session.close();
            }
    ...
    public T update(T entity) {
        ...
        try {
            ...
            throw new DataException("Can't update entity: " + entity, e);
        } finally {
            if (session != null) {
                session.close();
            }
